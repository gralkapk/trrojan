...
namespace trrojan::d3d12 {
    sphere_rt_data::sphere_rt_data()
        : _bbox({ glm::vec3(0.0f), glm::vec3(0.0f) })
        , _max_radius(0.0f)
    {}

    winrt::com_ptr<ID3D12Resource> sphere_rt_data::load(ID3D12GraphicsCommandList* command_list, D3D12_RESOURCE_STATES state,
    std::string const& path, std::uint32_t frame) {
        ...
        try {
            ...
            transition_resource(command_list, data_.get(),
            ...
            try {
                if (initial_color != list_header.colour_type
                    || initial_vertex != list_header.vertex_type) {
