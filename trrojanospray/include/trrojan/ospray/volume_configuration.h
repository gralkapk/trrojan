...
class TRROJANOSPRAY_API volume_configuration final {
public:
    ...
    explicit volume_configuration(const configuration& config);
    ...
    float volume_sampling_rate(void) const noexcept {
        return this->_volume_sampling_rate;
    }

    std::string xfer_func(void) const noexcept {
        return this->_xfer_func;
    }

    float fovy(void) const noexcept {
        return this->_fovy;
    }
